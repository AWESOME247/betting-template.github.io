<template>
    <div class="flex gap-2 px-2 place-items-center justify-between"><p class="sm:col-span-6">If it's not today's prediction</p> <button class="bg-green-400 w-fit p-2 rounded-md my-3 outline outline-green-200 hover:bg-green-600 inline-flex items-center gap-1 justify-center" @click="load()">{{ msg }} <img :class="{'hidden': msg === 'Refreshed!' }" src="/refresh-reload-svgrepo-com.svg" height="17" width="17" alt="refresh"></button></div>
    <table class="w-full border-collapse m-auto">
        ...
        <tbody class="border-collapse whitespace-nowrap">
            <template v-for="team in games" :key="team">
                <tr v-show="$route.path === '/free-vip-sure-accurate/2-5-multi-goals-prediction' ? multiGoals(team.tips?.split('(')[1].replace(')', '')) : true" :class="{ 'hidden': team.tips?.match('Draw')?.[0]}" class="even:bg-gray-900 even:text-gray-100">
                    ...
                    <td :class="{ 'hidden': $route.path === '/free-vip-sure-accurate/2-5-multi-goals-prediction' }" class="text-center py-4">{{ $route.path === '/free-vip-sure-accurate/king-of-correct-score' ? correctScore(team.home, team.tips?.split('(')[1].replace(')', ''), team.tips) : matchChecker(team.home, team.away, $route.path ===
                        '/free-vip-sure-accurate/sure-wins-only' ? team.tips : team.tips?.replace(/(Over)|(OVER)|(Ov)\s*/g,
                            '+').replace(/(ER)|(er)/g, '').replace(/(Home)|(HOME)\s*/g, '1').replace(/(Score)|(SCORE)\s*/g,
                                '+0.5').replace(/(Away)|(AWAY)\s*/g, '2').replace(/(Under)|(UNDER)|(UND)|(Un)\s*/g, '-').replace('-d', '-').trim()) }}
                    </td>
                    <td :class="{ 'hidden': $route.path !== '/free-vip-sure-accurate/2-5-multi-goals-prediction' }" class="text-center py-4">{{ $route.path === '/free-vip-sure-accurate/2-5-multi-goals-prediction' ? multiGoals(team.tips?.split('(')[1].replace(')', '')) : '' }}
                    </td>
